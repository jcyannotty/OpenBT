...
public:
   ...
   void setci(mxd invtau2_matrix, vxd beta_vec) {ci.invtau2_matrix.resize(invtau2_matrix.rows(),invtau2_matrix.rows()); ci.beta_vec.resize(beta_vec.rows());
      ci.invtau2_matrix=invtau2_matrix; ci.beta_vec=beta_vec; ci.diffpriors = true; for(size_t j=0;j<m;j++) mb[j].setci(invtau2_matrix,beta_vec);} //Set when using prior's that differ by function
