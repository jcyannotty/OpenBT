...
def read_in_preds(fpath, modelname, nummodels, q_upper, q_lower, readmean, readstd, readwts, ymean=0):
    # Init containers
    mdraws = []
    sdraws = []
    wdraws = {}
    ...
    for j in range(n_test):
        if readmean:
            pred_mean[j] = np.mean(mdraws[:, j] + ymean)
            pred_sd[j] = np.std(mdraws[:, j] + ymean, ddof=1) 
            pred_med[j] = np.quantile(mdraws[:, j] + ymean, 0.50)
            pred_lower[j] = np.quantile(mdraws[:, j] + ymean, q_lower)
            pred_upper[j] = np.quantile(mdraws[:, j] + ymean, q_upper)
...
def run_model(fpath, tc, cmd="openbtcli",
    ...
    KNOWN_COMMANDS = {
        ...
        "openbtmixingts","openbtmixingpredts"
    }
    ...
    elif shutil.which(cmd) is None and local_openbt_path == "":
        raise NotImplementedError("Add local_openbt_path.")
    elif local_openbt_path != "":
        cmd = shutil.which(cmd,path=local_openbt_path)
        if cmd is None:
            raise NotImplementedError("Openbt command line tools not found")
    ...
    if google_colab:
        ...
        try:
            ...
            if "ERROR" in str(sp.stdout):
                raise RuntimeError("Error in C++ \n" + str(sp.stdout))
            else:
                return sp
