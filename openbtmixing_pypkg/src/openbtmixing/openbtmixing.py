...
class Openbtmix():
    def __init__(self,**kwargs):
        ...
        self.selectp = False # Selection prior, keep false for now
    ...
    def get_prior(self):
        ...
        if self.rpath:
            out["rpath"] = self.rpath
            out["shape1"] = self.shape1
            out["shape2"] = self.shape2
            out["q"] = self.q
        return out
    ...
    def set_prior(
        ...
        if self.inform_prior:
            ...
            self.beta = 1 / self.ntree # correct for number of models in train
    ...
    def train(self, x_train: np.ndarray, y_train: np.ndarray, f_train: np.ndarray,
        ...
        self.beta = 1/(self.ntree*self.nummodels)
        ...
        if self.__dict__["xicuts"] is None:
            # Get the bounds
            xmax = np.round(np.max(x_train, axis=1),1) + 0.1
            xmin = np.round(np.min(x_train, axis=1),1) - 0.1
        ...
        run_params = [self.modeltype,self.xroot,self.yroot,self.fmean_out,self.ntree,self.ntreeh,
                        ...
                        self.selectp,self.rpath,self.gamma,self.q,self.shape1,self.shape2,
    ...
    def predict(self, x_test: np.ndarray, f_test: np.ndarray, ci: float = 0.95):
        ...
        res = ih.read_in_preds(self.fpath, self.modelname, self.nummodels, q_upper, q_lower, 
                               True, True, False)        
        return res
    ...
    def post_gamma(self):
        if self.rpath:
            ...
            print("post_gamma() is not valid for deterministic path models.")  
